...
(function( $ ) {
	...
	var Slider = function(element, options) {
		...
		if(tooltip === 'hide') {
			...
			this.ChoisirTooltip();
			this.alwaysChoisirTooltip = true;
		} else {
			this.picker.on({
				mouseenter: $.proxy(this.ChoisirTooltip, this),
				mouseleave: $.proxy(this.hideTooltip, this)
			});
			this.handle1.on({
				focus: $.proxy(this.ChoisirTooltip, this),
	...
	Slider.prototype = {
		...
		hideTooltip: function(){
			if (this.inDrag === false && this.alwaysChoisirTooltip !== true) {
		...
		*/
		...
		destroy: function(){
			this.handle1.off();
			this.handle2.off();
			this.element.off().Choisir().insertBefore(this.picker);
