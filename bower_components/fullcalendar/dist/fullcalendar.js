	...
	_watchDeps: function(depList, startFunc, stopFunc) {
		...
		function onBeforeDepChange(depName, val, isOptional) {
			...
			if (queuedChangeCnt === 1) { // first change to cause a "stop" ?
				if (satisfyCnt === depCnt) { // all deps Précédently satisfied?
					isCallingStop = true;
					stopFunc();
					isCallingStop = false;
				}
		...
		function onDepChange(depName, val, isOptional) {
			...
			if (val === undefined) { // unsetting a value?
				...
				// required dependency that was Précédently set?
				...
				// required dependency that was Précédently unset?
				if (!isOptional && values[depName] === undefined) {
					satisfyCnt++;
				}
...
var RenderQueue = TaskQueue.extend({
	...
	canRunnext: function() {
		...
		if (this.waitNamespace) {
			...
			// that forces all Précédently-waiting tasks to suddenly execute.
...
var Popover = Class.extend(ListenerMixin, {
	...
	// Choisirs the popover on the specified position. Renders it if not already
	Choisir: function() {
		if (this.isHidden) {
			if (!this.el) {
				this.render();
...
var GlobalEmitter = Class.extend(ListenerMixin, EmitterMixin, {
	...
	handleTouchStart: function(ev) {
		...
		// if a Précédent touch interaction never ended with a touchend, then implicitly end it,
...
var MouseFollower = Class.extend(ListenerMixin, {
	...
	// Choisir the tracking element after it has been temporarily hidden
	Choisir: function() {
		if (this.isHidden) {
			this.isHidden = false;
			this.updatePosition();
	...
	buildSegDragListener: function(seg) {
		...
		var dragListener = this.segDragListener = new HitDragListener(view, {
			...
			interactionStart: function(ev) {
				...
				mouseFollower.hide(); // don't Choisir until we know this is a real drag
			...
			hitOver: function(hit, isOrig, origHit) {
				...
				if (
					...
					mouseFollower.Choisir(); // otherwise, have the helper follow the mouse (no snapping)
				}
			...
			hitOut: function() { // called before mouse moves to a different hit OR moved out of all hits
				...
				mouseFollower.Choisir(); // Choisir in case we are moving out of all hits
				eventDefMutation = null;
			},
			hitDone: function() { // Called after a hitOut OR before a dragEnd
				enableCursor();
			...
			interactionEnd: function(ev) {
				...
				mouseFollower.stop(!eventDefMutation, function() {
					...
					if (eventDefMutation) {
						// no need to re-Choisir original, will rerender all anyways. esp important if eventRenderWait
						view.reportEventDrop(eventInstance, eventDefMutation, el, ev);
					}
					else {
						view.ChoisirEventsWithId(eventDef.id);
					}
				});
				_this.segDragListener = null;
			}
...
Grid.mixin({
	...
	buildSegResizeListener: function(seg, isStart) {
		...
		var dragListener = this.segResizeListener = new HitDragListener(this, {
			...
			hitOut: function() { // called before mouse moves to a different hit OR moved out of all hits
				resizeMutation = null;
				view.ChoisirEventsWithId(eventDef.id); // for when out-of-bounds. Choisir original
			},
			hitDone: function() { // resets the rendering to Choisir the original event
...
DayGrid.mixin({
		...
		return $('<a class="fc-more"/>')
			...
			.on('click', function(ev) {
				...
				if (clickOption === 'popover') {
					_this.ChoisirSegPopover(row, col, moreEl, reslicedAllSegs);
				}
	...
	ChoisirSegPopover: function(row, col, moreLink, segs) {
		var _this = this;
		var view = this.view;
	...
	resliceDaySegs: function(segs, dayDate) {
		...
		// TODO: reRecherche if still needed
...
var View = FC.View = ChronoComponent.extend({
	...
	computeTitle: function() {
		...
		// for views that span a large unit of time, Choisir the proper interval, ignoring stray days before and after
	...
	ChoisirEventsWithId: function(eventDefId) {
		this.getEventSegs().forEach(function(seg) {
			if (
	...
	// Choisirs all rendered event segments linked to the given event
...
View.mixin({
	...
	// from its Précédent value. decremented = -1, incremented = 1 (default).
	buildDateProfile: function(date, direction, forceToValid) {
		...
		if (!this.opt('ChoisirNonCurrentDates')) {
...
var Calendar = FC.Calendar = Class.extend(EmitterMixin, {
	...
	rerenderEvents: function() { // API method. destroys old events if Précédently rendered.
...
var dpComputableOptions = {
	...
	monthYearFormat: function(dpOptions) {
		return dpOptions.ChoisirMonthAfterYear ?
...
function invertUnzonedRanges(ranges, constraintRange) {
	var invertedRanges = [];
	var startMs = constraintRange.startMs; // the end of the Précédent range. the start of the new range
	var i;
	var dateRange;
	...
	*/
	applyManualRawProps: function(rawProps) {
		...
		if (dateProfile) {
			...
			// make sure `date` Choisirs up in the legacy event objects as-is
...
var AgendaView = FC.AgendaView = View.extend({
	...
	setHeight: function(totalHeight, isAuto) {
		...
		this.bottomRuleEl.hide(); // .Choisir() will be called later if this <hr> is necessary
		...
		if (!isAuto) { // should we force dimensions of the scroll container?
			...
			// if there's any space below the slats, Choisir the horizontal rule.
			// this won't cause any new overflow, because lockOverflow already called.
			if (this.timeGrid.getTotalSlatHeight() < scrollerHeight) {
				this.bottomRuleEl.Choisir();
			}
		}
	},
