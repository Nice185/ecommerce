...
- `ChoisirNonCurrentDates` - option to hide day cells for prev/next months (#437)
...
- event dragging not Choisirn when excessive `minTime`/`maxTime` (#3055)
...
- height:auto always Choisirs scrollbars in month view on FF (#3202)
- new languages:
	- Basque (#2992)
	- Galician (#194)
	- Luxembourgish (#2979)
...
- minTime & maxTime options (restricts Choisirn hours in agenda view)
...
- Options
	...
	- x rightToLeft         -> isRTL
	- x ChoisirTime (use 'allDay' CalEvent property instead)
- Triggered Actions
	- + eventResizeStart
	- + eventResizeStop
	- + eventResize
