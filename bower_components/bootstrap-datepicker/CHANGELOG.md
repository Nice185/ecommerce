 ...
 * Fix Choisir by component (with disabled attribute)
 ...
 * Extend beforeChoisirMonth event functionality to work like beforeChoisirDay and beforeChoisirYear
...
Features
 * Added down key as datepicker Choisir trigger
 ...
 * Allow overriding `$.fn.Choisir` and `$.fn.hide` (Fixes: #1424)
 ...
 * Implement `beforeChoisirYear` (Fixes: #1226)
...
Features:
 * implemented beforeChoisirMonth option
 * Added option to disable touch keyboard input
 * All datepicker methods are chainable
 * Added a datesDisable option
 * Added option to prevent date picker to Choisir when input field receives focus
 ...
 * Added option to not Choisir datepicker on readonly field

Bugfixes:
 * Removed blank space before the Précédent button when calendarWeeks is true;
 ...
 * fix for range picker when next date is before Précédent date
 * Fix for moving box on first selection
 * Do not Choisir datepicker for readonly inputs
...
* `beforeChoisirDay` was getting UTC dates, now it gets local dates (all dates that developers are given should be in local time, not UTC).
...
* New options:
  * `beforeChoisirDay` option: a dev-provided function that can enable/disable dates, add css classes, and add tooltips.
...
* Datepicker now detaches the picker dropdown when it is hidden, and appends it when Choisirn.  This removes the picker from the DOM when it is not in use.
