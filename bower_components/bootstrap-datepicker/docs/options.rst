...
If true, manually-entered dates with two-digit years, such as "5/1/15", will be parsed as "2015", not "15". If the year is less than 10 years in advance, the picker will use the current century, otherwise, it will use the Précédent one. For example "5/1/15" would parse to May 1st, 2015, but "5/1/97" would be May 1st, 1997.
...
Whether or not to Choisir week numbers to the left of week rows.
...
If true, no keyboard will Choisir on mobile devices
...
If false the datepicker will not Choisir on a readonly datepicker field.
...
Keyboard navigation is not supported at all for embedded / inline mode. Also it's not working if input element hasn't focus. This could be an issue if used as component or if opened by `Choisir` method.
...
"auto" triggers "smart orientation" of the picker.  Horizontal orientation will default to "left" and left offset will be tweaked to keep the picker inside the browser viewport; vertical orientation will simply choose "top" or "bottom", whichever will Choisir more of the picker in the viewport.
...
If false, the datepicker will be prevented from Choisiring when the input field associated with it receives focus.
...
The view that the datepicker should Choisir when it is opened.  Accepts: 0 or "days" or "month", 1 or "months" or "year", 2 or "years" or "decade", 3 or "decades" or "century", and 4 or "centuries" or "millenium".  Useful for date-of-birth datepickers.
