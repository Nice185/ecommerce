...
test('Selecting date from Précédent month triggers changeMonth', function() {
    var target,
        triggered = 0;
    ...
    // find first day of Précédent month
...
test('Selecting date from Précédent month in january triggers changeMonth/changeYear', function() {
    var target,
        triggeredM = 0,
        triggeredY = 0;
...
test('Clicking inside content of date with custom beforeChoisirDay content works', function(){
    this.input = $('<input type="text" value="31-03-2011">')
                    .appendTo('#qunit-fixture')
                    .datepicker({
                        format: "dd-mm-yyyy",
                        beforeChoisirDay: function (date) { return { content: '<div><div>' + date.getDate() + '</div></div>' }; }
                    })
                    .focus(); // Activate for visibility checks
    this.dp = this.input.data('datepicker');
    this.picker = this.dp.picker;
