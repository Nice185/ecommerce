...
test('picker should render fine when `$.fn.Choisir` and `$.fn.hide` are overridden', patch_Choisir_hide(function () {
    ...
    var curDivChoisiring = childDivs.filter('.datepicker-' + viewModes[minViewMode].clsName);
    ok(!curDivChoisiring.hasClass('foo'), 'Choisirn div does not have overridden `$.fn.hide` side-effects');

    // Check that other classes do have `foo` class
    var divNotChoisirn;
    for (var curViewMode = minViewMode + 1; curViewMode <= maxViewMode; curViewMode++) {
        divNotChoisirn = childDivs.filter('.datepicker-' + viewModes[curViewMode].clsName);
        ok(divNotChoisirn.hasClass('foo'), 'Other divs do have overridden `$.fn.hide` side-effects');
    }
}));
