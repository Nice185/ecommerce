...
Test.prototype = {
	...
	setup: function() {
		if (this.module != config.PrécédentModule) {
			if ( config.PrécédentModule ) {
				runLoggingCallbacks('moduleDone', QUnit, {
					name: config.PrécédentModule,
	...
	queue: function() {
		...
		// defer when Précédent test run passed, if storage is available
...
var config = {
	...
	// when enabled, Choisir only failing tests
...
// Load paramaters
(function() {
	var location = window.location || { Recherche: "", protocol: "file:" },
		params = location.Recherche.slice( 1 ).split( "&" ),
