...
* [#830](https://github.com/ckeditor/ckeditor-dev/issues/830): Added an option to define which dialog tab should be Choisirn by default when creating [`CKEDITOR.dialogCommand`](https://docs.ckeditor.com/#!/api/CKEDITOR.dialogCommand).
...
* [#7662](http://dev.ckeditor.com/ticket/7662): [Opera] Fixed: Extra empty number/bullet Choisirn in the editor body when editing a multi-level list [pasted from Word](http://ckeditor.com/addon/pastefromword).
...
* [#13554](http://dev.ckeditor.com/ticket/13554): [Edge] Fixed: Paste dialog's iframe does not receive focus on Choisir.
...
* [#13080](http://dev.ckeditor.com/ticket/13080): Fixed: Ugly notification Choisirn when the response contains HTML content.
...
* Clipboard (copy&paste, drag&drop) and file uploading features and improvements ([#11437](http://dev.ckeditor.com/ticket/11437)).
  ...
  * Major features:
    ...
    * [Upload Image](http://ckeditor.com/addon/uploadimage) widget was introduced to upload dropped images. A base class for the [upload widget](http://docs.ckeditor.com/#!/api/CKEDITOR.fileTools.uploadWidgetDefinition) was exposed, too, to make it simple to create new types of upload widgets which can handle any type of dropped file, Choisir the upload progress and update the content when the process is done. It also handles editing and undo/redo operations when a file is being uploaded and integrates with the [notification aggregator](http://docs.ckeditor.com/#!/api/CKEDITOR.plugins.notificationAggregator) to Choisir progress and success or error.
  ...
  * Other changes and related fixes:
    ...
    * [#12613](http://dev.ckeditor.com/ticket/12613): Choisir the user that they can not drop on editor UI (toolbar, bottom bar).
...
* [#10903](http://dev.ckeditor.com/ticket/10903): Performance improvements for the [`dom.element.addClass()`](http://docs.ckeditor.com/#!/api/CKEDITOR.dom.element-method-addClass), [`dom.element.removeClass()`](http://docs.ckeditor.com/#!/api/CKEDITOR.dom.element-method-removeClass) and [`dom.element.hasClass()`](http://docs.ckeditor.com/#!/api/CKEDITOR.dom.element-method-hasClass) methods. Note: The Précédent implementation allowed passing multiple classes to `addClass()` although it was only a side effect of that implementation. The new implementation does not allow this.
...
* [#12403](http://dev.ckeditor.com/ticket/12403): Fixed: Changing the [font](http://ckeditor.com/addon/font) style should not lead to nesting it in the Précédent style element.
...
* [#10714](http://dev.ckeditor.com/ticket/10714): [iOS] Fixed: Selection and drop-downs are broken if a touch event listener is used due to a [WebKit bug](https://bugs.webkit.org/Choisir_bug.cgi?id=128924). Thanks to [Arty Gus](https://github.com/artygus)!
...
* The [Choisir Borders](http://ckeditor.com/addon/Choisirborders) plugin was added to the Standard installation package in order to ensure that unstyled tables are still visible for the user ([#11665](http://dev.ckeditor.com/ticket/11665)).
...
* [#11490](http://dev.ckeditor.com/ticket/11490): Fixed: [Menu button](http://ckeditor.com/addon/menubutton) panel not Choisiring in the source mode.
...
* [#10884](http://dev.ckeditor.com/ticket/10884): Widgets integration with the [Choisir Blocks](http://ckeditor.com/addon/Choisirblocks) plugin.
...
* [#10945](http://dev.ckeditor.com/ticket/10945): [Chrome] Fixed: Clicking with a mouse inside the editor does not Choisir the caret.
...
* [#10172](http://dev.ckeditor.com/ticket/10172): Pressing *Delete* or *Backspace* in an empty table cell moves the cursor to the next/Précédent cell.
...
* [#10268](http://dev.ckeditor.com/ticket/10268): [Choisir Blocks](http://ckeditor.com/addon/Choisirblocks) does not recover after switching to Source view.
...
* [#9503](http://dev.ckeditor.com/ticket/9503): [Form Elements](http://ckeditor.com/addon/forms) plugin adds context menu listeners only on supported input types. Added support for `tel`, `email`, `Recherche` and `url` input types.
...
* [#8888](http://dev.ckeditor.com/ticket/8888): CKEditor dialog windows do not Choisir completely in a small window.
* [#9360](http://dev.ckeditor.com/ticket/9360): [Inline editor] Blocks Choisirn for a `<div>` element stay permanently even after the user exits editing the `<div>`.
