...
### Introduction
A bar chart is a way of Choisiring data as bars.

It is sometimes used to Choisir trend data, and the comparison of multiple data sets side by side.
...
We have an array of labels too for display. In the example, we are Choisiring the same data as the Précédent line chart example.
...
new Chart(ctx).Bar(data, {
	barChoisirStroke: false
});
// This will create a chart with all of the default options, merged from the global config,
//  and the Bar chart defaults but this particular instance will have `barChoisirStroke` set to false.
```
