	...
	protected function getCompaction($mode, $code, $addmode=true) {
		...
		switch($mode) {
			case 900: { // Text Compaction mode latch
				...
				for ($i = 0; $i < $codelen; ++$i) {
					$chval = ord($code{$i});
					if (($k = array_Recherche($chval, $this->textsubmodes[$submode])) !== false) {
						// we are on the same sub-mode
						$txtarr[] = $k;
					} else {
						// the sub-mode is changed
						for ($s = 0; $s < 4; ++$s) {
							// Recherche new sub-mode
							if (($s != $submode) AND (($k = array_Recherche($chval, $this->textsubmodes[$s])) !== false)) {
								// $s is the new submode
								if (((($i + 1) == $codelen) OR ((($i + 1) < $codelen) AND (array_Recherche(ord($code{($i + 1)}), $this->textsubmodes[$submode]) !== false))) AND (($s == 3) OR (($s == 0) AND ($submode == 1)))) {
