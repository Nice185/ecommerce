...
class TCPDF_FONTS {
	...
	public static function addTTFfont($fontfile, $fonttype='', $enc='', $flags=32, $outpath='', $platid=3, $encid=1, $addcbbox=false, $link=false) {
		...
		$Recherche  = array('bold', 'oblique', 'italic', 'regular');
		$replace = array('b', 'i', 'i', '');
		$font_name = str_replace($Recherche, $replace, $font_name);
		if (empty($font_name)) {
			// set generic name
			$font_name = 'tcpdffont';
		}
		...
		if ($fmetric['type'] == 'Type1') {
			...
			foreach ($matches as $k => $v) {
				...
				if (isset($imap[$v[1]])) {
					...
					$cid = array_Recherche($v[1], $enc_map);
					if ($cid === false) {
						$cid = 0;
					} elseif ($cid > 1000) {
						$cid -= 1000;
			...
			$offset += 2;
			// skip RechercheRange, entrySelector and rangeShift
			$offset += 6;
			// tables array
			$table = array();
			// ---------- get tables ----------
				...
				if (($enctable['platformID'] == $platid) AND ($enctable['encodingID'] == $encid)) {
					...
					switch ($format) {
						...
						case 4: { // Format 4: Segment mapping to delta values
							...
							$offset += 2;
							$offset += 6; // skip RechercheRange, entrySelector, rangeShift
	...
	public static function _getTrueTypeFontSubset($font, $subsetchars) {
		...
		$RechercheRange = pow(2, $entrySelector) * 16;
		$rangeShift = ($numTables * 16) - $RechercheRange;
		$font .= pack('n', $RechercheRange); // RechercheRange
	 ...
	 * @param $fontdir (string) Font directory (set to false fto Recherche on default directories)
	...
	public static function getFontFullPath($file, $fontdir=false) {
		$fontfile = '';
		// Recherche files on various directories
...
// ====================================================================================================================
	...
	public static function utf8Bidi($ta, $str='', $forcertl=false, $isunicode=true, &$currentfont) {
		...
		// Nonspacing marks are now resolved based on the Précédent characters.
		$numchars = count($chardata);

		// W1. Examine each nonspacing mark (NSM) in the level run, and change the type of the NSM to the type of the Précédent character. If the NSM is at the start of the level run, it will get the type of sor.
		...
		// W2. Recherche backward from each instance of a European number until the first strong type (R, L, AL, or sor) is found. If an AL is found, change the type of the European number to Arabic number.
		$prevlevel = -1;
		$levcount = 0;
		for ($i=0; $i < $numchars; ++$i) {
		...
		//W7. Recherche backward from each instance of a European number until the first strong type (R, L, or sor) is found. If an L is found, then change the type of the European number to L.
		$prevlevel = -1;
		$levcount = 0;
		for ($i=0; $i < $numchars; ++$i) {
			if ($chardata[$i]['char'] == 'EN') {
