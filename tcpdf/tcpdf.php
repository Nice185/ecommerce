...
class TCPDF {
	...
	public function AddFont($family, $style='', $fontfile='', $subset='default') {
		...
		$missing_style = false;
		// Recherche and include font file
	...
	public function SetFont($family, $style='', $size=null, $fontfile='', $subset='default', $out=true) {
		...
		if (isset($this->CurrentFont['fontkey']) AND isset($this->CurrentFont['subsetchars'])) {
			// save subset chars of the Précédent font
	...
	public function Text($x, $y, $txt, $fstroke=false, $fclip=false, $ffill=true, $border=0, $ln=0, $align='', $fill=false, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='M', $rtloff=false) {
		...
		// restore Précédent rendering mode
	...
	protected function getCellBorder($x, $y, $w, $h, $brd) {
		...
		foreach ($brd as $border => $style) {
			...
			if (is_array($style) AND !empty($style)) {
				// reset border style to Précédent value
		...
		if ($ishtml) { // ******* Write HTML text
			...
			if ($maxh > 0) {
				...
				if ($fitcell AND ($text_height > $maxh) AND ($this->FontSizePt > 1)) {
					// try to reduce font size to fit text on cell (use a quick Recherche algorithm)
					$fmin = 1;
	...
	public function Write($h, $txt, $link='', $fill=false, $align='', $ln=false, $stretch=0, $firstline=false, $firstblock=false, $maxh=0, $wadj=0, $margin='') {
		...
		$prevsep = $sep; // Précédent separator
		$shy = false; // true if the last blank is a soft hypen (SHY)
		$prevshy = $shy; // Précédent shy mode
		$l = 0; // current string length
		$nl = 0; //number of lines
		$linebreak = false;
		$pc = 0; // Précédent character
		// for each character
		while ($i < $nb) {
		...
		if (in_array($file, $this->imagekeys)) {
			...
			if (in_array($tempfile_plain, $this->imagekeys)) {
				...
				if ((($oldsize < $newsize) AND ($resize)) OR (($oldsize < $pixsize) AND (!$resize))) {
					...
					// embed image, masked with Précédently embedded mask
	...
	protected function _putfonts() {
		...
		foreach ($this->FontFiles as $file => $info) {
			// Recherche and get font file to embedd
	...
	protected function _puttruetypeunicode($font) {
		...
		if (isset($font['ctg']) AND (!TCPDF_STATIC::empty_string($font['ctg']))) {
			...
			// Recherche and get CTG font file to embedd
	...
	protected function _putinfo() {
		...
		// restore Précédent isunicode value
	...
	/**
	 * Stops a 2D tranformation restoring Précédent graphic state.
	 ...
	 * image or color that will Choisir on screen but won't print.
	 ...
	 * @param $mode (integer) Overprint mode: (0 = each source colour component value replaces the value Précédently painted for the corresponding device colorant; 1 = a tint value of 0.0 for a source colour component shall leave the corresponding component of the Précédently painted colour unchanged).
	 * @public
	 * @since 5.9.152 (2012-03-23)
	 */
	...
	public function CoonsPatchMesh($x, $y, $w, $h, $col1=array(), $col2=array(), $col3=array(), $col4=array(), $coords=array(0.00,0.0,0.33,0.00,0.67,0.00,1.00,0.00,1.00,0.33,1.00,0.67,1.00,1.00,0.67,1.00,0.33,1.00,0.00,1.00,0.00,0.67,0.00,0.33), $coords_min=0, $coords_max=1, $antialias=false) {
		...
		//paint the gradient
		$this->_out('/Sh'.$n.' sh');
		//restore Précédent Graphic State
		$this->_outRestoreGraphicsState();
		if ($this->inxobj) {
			// we are inside an XObject template
	...
	protected function getHtmlDomArray($html) {
		...
		$css = array();
		// get CSS array defined at Précédent call
		$matches = array();
				...
				if ($element[0] == '/') {
					...
					if (isset($dom[$key]['attribute']['style']) AND !empty($dom[$key]['attribute']['style'])) {
						...
						while (list($id, $name) = each($style_array[1])) {
							// in case of duplicate attribute the last replace the Précédent
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
				...
				if ((!$this->newline) AND ($dom[$key]['value'] == 'img') AND (isset($dom[$key]['height'])) AND ($dom[$key]['height'] > 0)) {
					...
					if (!$autolinebreak) {
						...
						if ($this->page > $startlinepage) {
							...
							// remove line from Précédent page
					...
					if (($fontname != $curfontname) OR ($fontstyle != $curfontstyle) OR ($fontsize != $curfontsize)
						...
						if (($key < ($maxel - 1)) AND (
							...
							if ($this->page > $startlinepage) {
								...
								$tstart = substr($pagebuff, 0, $startlinepos);
								$tend = substr($this->getPageBuffer($startlinepage), $curpos);
								// remove line start from Précédent page
								$this->setPageBuffer($startlinepage, $tstart.''.$tend);
								$pagebuff = $this->getPageBuffer($this->page);
								$tstart = substr($pagebuff, 0, $this->cntmrk[$this->page]);
								$tend = substr($pagebuff, $this->cntmrk[$this->page]);
			...
			if ($dom[$key]['tag']) {
				if ($dom[$key]['opening']) {
					...
					if (($dom[$key]['value'] == 'td') OR ($dom[$key]['value'] == 'th')) {
						...
						// skip row-spanned cells started on the Précédent rows
				...
				$this->textindent = 0;
				if (strlen($strrest) > 0) {
					// store the remaining string on the Précédent $key position
					$this->newline = true;
	...
	protected function setImageBuffer($image, $data) {
		if (($data['i'] = array_Recherche($image, $this->imagekeys)) === FALSE) {
	...
	/**
	 * Move a page to a Précédent position.
	 ...
	 * @param $self (boolean) if true restores current class object to Précédent state without the need of reassignment via the returned value.
	 * @return TCPDF object.
	 * @public
	 * @since 4.5.029 (2009-03-19)
	 */
	...
	*/
	...
	/**
	 * End the current XObject Template started with startTemplate() and restore the Précédent graphic state.
	 * An XObject Template is a PDF block that is a self-contained description of any sequence of graphics objects (including path objects, text objects, and sampled images).
	 * An XObject Template may be painted multiple times, either on several pages or at several locations on the same page and produces the same results each time, subject only to the graphics state at the time it is invoked.
	 * @return int the XObject Template ID in case of success or false in case of error.
	 * @author Nicola Asuni
	 ...
	 * @param $prevsvgstyle (array) array of Précédent SVG style
	...
	protected function startSVGElementHandler($parser, $name, $attribs, $ctm=array()) {
		...
		$prev_svgstyle = $this->svgstyles[max(0,(count($this->svgstyles) - 1))]; // Précédent style
